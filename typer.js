...
var WordView = Backbone.View.extend({
	initialize: function() {
		$(this.el).css({position:'absolute', width: '100%', 'min-width': '700px'});
				...
				for(var i = 0;i < words.length;i++) {
					...
					if(string.toLowerCase().indexOf(typed_string.toLowerCase()) == 0) {
						...
						if(typed_string.length == string.length) {
							$('input[name="typerScoring"]').val(myScore + 10);
							myScore = parseInt($('input[name="typerScoring"]').val());
							$(this).val('');
						}
					} else {
						if (word.get('highlight') > 0 && typed_string.length > 2) {
							$('input[name="typerScoring"]').val(myScore - 5);
							myScore = parseInt($('input[name="typerScoring"]').val());
						}
						word.set({highlight:0});
					}
				}
			}).attr('name', 'typerWords');
